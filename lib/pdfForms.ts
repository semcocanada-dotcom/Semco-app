...
function buildExpenseHtml(data: ExpensePdfInput): string {
  ...
  while (paddedRows.length < MIN_ROWS) paddedRows.push(null as unknown as ExpensePdfRow);
  ...
  const tableRows = paddedRows.map((row) => {
    ...
    return `<tr>
      <td class="td-date">${fmtDate(row.expense_date)}</td>
      <td class="td-purpose">${row.provider_name}</td>
      <td class="td-purpose">${row.category_label}${row.description ? ' — ' + row.description : ''}</td>
      <td class="td-num">$${Number(row.amount).toFixed(2)}</td>
    </tr>`;
  }).join('');
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: Arial, Helvetica, sans-serif; font-size: 11px; color: #000; padding: 28px 32px; background: #fff; }
  .title-main { font-size: 20px; font-weight: 400; color: #1a1a1a; margin-bottom: 2px; }
  .title-sub { font-size: 14px; font-weight: 700; color: #215732; margin-bottom: 4px; }
  .instructions { font-size: 11px; color: #333; margin-bottom: 18px; }
  .instructions a { color: #215732; }
  .section-heading { font-size: 12px; font-weight: 700; color: #215732; margin-bottom: 4px; margin-top: 14px; }
  .field-row { display: flex; border-bottom: 1px solid #000; margin-bottom: 14px; }
  .field-cell { flex: 1; padding-bottom: 4px; padding-right: 12px; }
  .field-cell-wide { flex: 2; }
  .field-label { font-size: 9px; color: #555; margin-top: 4px; display: block; }
  .field-value { font-size: 12px; min-height: 16px; }
  .divider { border: none; border-top: 1px solid #ccc; margin: 14px 0; }
  .month-row { font-size: 12px; font-weight: 700; color: #215732; margin-bottom: 10px; }
  .month-value { font-weight: 400; color: #000; border-bottom: 1px solid #000; display: inline-block; min-width: 120px; padding-left: 4px; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 0; }
  th { background: #215732; color: #fff; font-size: 10px; font-weight: 700; padding: 6px; text-align: center; vertical-align: middle; border: 1px solid #215732; }
  ...
  .td-num { width: 88px; text-align: right; }
  .total-row { display: flex; justify-content: flex-end; align-items: center; border: 1px solid #bbb; border-top: none; }
  .total-label { font-size: 11px; font-weight: 700; padding: 6px 10px; text-align: right; flex: 1; }
  .total-value { font-size: 11px; font-weight: 700; padding: 6px 10px; width: 100px; border-left: 1px solid #bbb; text-align: right; }
  .sig-row { display: flex; margin-top: 16px; gap: 32px; }
  .sig-cell { flex: 1; }
  .sig-line { border-bottom: 1px solid #000; min-height: 28px; margin-bottom: 4px; font-size: 12px; padding-bottom: 2px; }
  .sig-label { font-size: 9px; color: #555; }
  .asterisk-note { font-size: 10px; color: #333; margin-top: 6px; margin-bottom: 14px; }
  .footer { display: flex; justify-content: space-between; align-items: flex-end; margin-top: 28px; }
  .footer-site { font-size: 13px; font-weight: 700; color: #215732; font-style: italic; }
  .footer-logo { font-size: 18px; font-weight: 700; color: #215732; font-style: italic; }
  .footer-logo span { color: #F4C430; font-style: normal; }
</style>
</head>
<body>
  <div class="title-main">Autism Spectrum Disorder Individualized Funding (ASD-IF)</div>
  <div class="title-sub">Monthly Expense Claim Form: Expense Information</div>
  <p class="instructions">
    Instructions to complete this form can be found on
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="th-left">Provider / Payee</th>
        <th class="th-left">Service Type &amp; Description</th>
