import { SIP_MANUAL_PAGES } from '../../knowledge/sip-manual';
...
export function searchSipManual(query: string, limit = DEFAULT_LIMIT): ManualKnowledgeHit[] {
  ...
  if (terms.length === 0 || SIP_MANUAL_PAGES.length === 0) return [];

  const hits = SIP_MANUAL_PAGES.map((page) => {
...
export function buildSipManualContext(hits: ManualKnowledgeHit[]): string {
  ...
  return hits
    .map((hit) => `[${hit.sourceDocument} • p. ${hit.pageNumber}]\n${hit.excerpt}`)
    .join('\n\n---\n\n');
}
...
export function formatSipManualResponse(hits: ManualKnowledgeHit[]): string {
  ...
  const bulletLines = hits.map(
    (hit) => `- p. ${hit.pageNumber}: ${hit.excerpt}`,
  );

  return [
    'Based on the SIP manual:',
    '',
    ...bulletLines,
  ].join('\n');
}
