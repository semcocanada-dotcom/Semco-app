...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .notice { border: 2px solid #D64045; border-radius: 8px; background: #FFF5F5; color: #5B2430; padding: 10px 12px; margin-bottom: 20px; }
  .notice strong { display: block; color: #A51D25; font-size: 13px; text-transform: uppercase; letter-spacing: .35px; margin-bottom: 3px; }
...
<div class="header">
  <div class="header-left">
    <h1>Autism Fund Tracker Grant Worksheet</h1>
    <p>Independent funding and expense summary for personal recordkeeping</p>
  </div>
...
<div class="footer">
  <p><strong>${esc(UNOFFICIAL_WORKSHEET_NOTICE)}</strong></p>
  <p>${esc(INDEPENDENCE_NOTICE)}</p>
  <p>Parent/Guardian: ${esc(parentName)} · Generated ${format(new Date(), 'MMMM d, yyyy \'at\' h:mm a')}</p>
  <p>Verify current requirements and submit any required government form separately at https://autismfunding.saskatchewan.ca/.</p>
</div>
    ...
    try {
      ...
      if (canShare) {
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: `Grant worksheet — ${activeChild.name}`,
          UTI: 'com.adobe.pdf',
        });
      } else {
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <>
              ...
              >
                {generating ? (
                  ...
                  <>
                    ...
                    <View>
                      <Text style={s.generateTitle}>Generate PDF Worksheet</Text>
                      <Text style={s.generateSub}>Personal recordkeeping — not an official submission</Text>
                    </View>
                  </>
                )}
              </LinearGradient>
            ...
            {/* What's in the PDF */}
            <Text style={s.sectionLabel}>PDF Worksheet Includes</Text>
